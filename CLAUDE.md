...
In `app/javascript/controllers/`, auto-registered via `eagerLoadControllersFrom` (`app/javascript/controllers/index.js`). Any non-trivial view-side JS behavior (chart tweaks, DOM manipulation, event handling beyond `data-action`) belongs in a Stimulus controller — do not write inline `<script>` blocks in views. Notable ones: `autosubmit_controller.js`, `element_removal_controller.js`, `press_controller.js` (button press scale/brightness animation), `reveal_controller.js` (scroll-in reveal + count-up), `chart_legend_controller.js` (Chartkick/Chart.js legend enhancement: dims other series on legend hover, and — opt in via the `resizeOnHide` value — makes hiding a series via the legend remove its slot from the x-axis instead of leaving a gap; used on the `ActivitiesPerCategoryPresenter` bar chart and the time-series line chart in `statistics/show`).
